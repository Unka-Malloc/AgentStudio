...
describe("server-web small API clients final extra coverage", () => {
  ...
  it("builds external service requests and exposes option vocabularies", () => {
    ...
    expect(externalServiceModelProtocolOptions.map((option) => option.value)).toContain("custom-json-http");
    expect(externalServiceMcpTransportOptions.map((option) => option.value)).toContain("stdio");
