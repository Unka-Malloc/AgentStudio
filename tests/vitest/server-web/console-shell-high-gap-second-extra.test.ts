...
function createExternalServicesControllerFixture(overrides: Record<string, unknown> = {}) {
  ...
  return {
    ...
    serviceActiveToolCount: (service: any) => service.externalMcp?.tools?.length || 0,
    serviceCandidateToolCount: (service: any) => Number(service.externalMcp?.candidateToolCount || 0),
    serviceToolAdoptionLabel: (service: any) => Number(service.externalMcp?.candidateToolCount || 0) > 0 ? "候选待采纳" : "工具已采纳",
    serviceActiveToolReviewRows: (service: any) =>
...
describe("external services controller high gap second extra", () => {
  it("covers stable fallback branches for upstream resolution and discovery labels", async () => {
    ...
    expect(controller.serviceDiscoveryLabel(httpsService as any)).toBe("HTTPS 服务");
