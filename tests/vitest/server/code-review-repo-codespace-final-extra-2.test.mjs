...
describe("gerrit, repo-operations, and codespace final extra coverage", () => {
  ...
  it("falls back to raw Gerrit errors when the response body is non-JSON or empty", async () => {
    ...
    const textError = await executeGerritCommonOperation({
      mode: "read",
      input: {
        action: "changes.get",
        changeId: "I404"
    ...
    const emptyBodyError = await executeGerritCommonOperation({
      mode: "read",
      input: {
        action: "changes.get",
        changeId: "I404-empty"
    ...
    expect(emptyBodyError).toMatchObject({
      ok: false,
      error: "Not Found",
      gerrit: {
        status: 404
  ...
  it("rejects empty repo operations and returns unavailable status for unknown targets", async () => {
    const emptyOperation = await executeRepoOperation({
      input: {}
    });

    expect(emptyOperation).toMatchObject({
      ok: false,
      status: 400,
      error: {
        code: "unsupported_repo_operation",
        message: "Unsupported repo operation: (empty)"
      }
    });
    expect(emptyOperation.error.details.allowedOperations).toEqual(
      expect.arrayContaining(["repo.status", "repo.tree.list", "repo.review.approve"])
    );

    queueResolvedRepo();
    const unavailable = await executeRepoOperation({
  ...
  it("maps unknown Gerrit sync statuses to open/notSubmitted instead of failing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pact-codespace-status-extra-"));
    try {
      await fs.mkdir(path.join(root, "code-management"), { recursive: true });
      await fs.writeFile(
      ...
      executeGerritCommonOperationMock.mockResolvedValueOnce({
        ...
        mode: "read",
        result: {
          status: "some-unrecognized-state",
          submitStatus: "anything",
          change_id: "I999999",
          _number: 99,
