...
import { assertPactiumToken, pactiumToken, resolveDataDir, resolveWithin } from "./paths.js";

export const CHECKPOINT_TREE_SCHEMA_VERSION = "v0.1.0:pactium:checkpoint-tree-schema-1";
export const CHECKPOINT_TREE_PROTOCOL_VERSION = "v0.1.0:pactium:checkpoint-tree-1";
...
function checkpointTreeRoot(userDataPath) {
  return resolveWithin(resolveDataDir(userDataPath), "checkpoint-trees");
}

function checkpointTreePath(userDataPath, treeId) {
  assertPactiumToken(treeId, "checkpoint_tree");
...
export function checkpointTreeId(kind, ...parts) {
  return pactiumToken("checkpoint_tree", kind, ...parts);
}
...
export async function diffCheckpointTree({
  ...
  return {
    protocolVersion: CHECKPOINT_TREE_PROTOCOL_VERSION,
...
export async function restoreCheckpointTree({
  ...
  assertPactiumToken(normalizedTreeId, "checkpoint_tree");
...
function withDataDir(dataDir, input = {}) {
  return {
    ...input,
    userDataPath: resolveDataDir(input.userDataPath || input.dataDir || dataDir)
  };
}

export function createCheckpointTreeStore({ dataDir = "" } = {}) {
  ...
  return Object.freeze({
    ...
    id: checkpointTreeId,
    path(treeId) {
      return getCheckpointTreePath(resolvedDataDir, treeId);
    },
    list(input = {}) {
      return listCheckpointTrees(withDataDir(resolvedDataDir, input));
    },
    load(input = {}) {
      return loadCheckpointTree(withDataDir(resolvedDataDir, input));
    },
    startTree(input = {}) {
      return startCheckpointTree(withDataDir(resolvedDataDir, input));
    },
    upsertNode(input = {}) {
      return upsertCheckpointNode(withDataDir(resolvedDataDir, input));
    },
    finishTree(input = {}) {
      return finishCheckpointTree(withDataDir(resolvedDataDir, input));
    },
    diffTree(input = {}) {
      return diffCheckpointTree(withDataDir(resolvedDataDir, input));
    },
    queryScope(input = {}) {
      return queryCheckpointScope(withDataDir(resolvedDataDir, input));
    },
    previewRestore(input = {}) {
      return previewCheckpointRestore(withDataDir(resolvedDataDir, input));
    },
    restore(input = {}) {
      return restoreCheckpointTree(withDataDir(resolvedDataDir, input));
    },
    deleteTree(input = {}) {
      return deleteCheckpointTree(withDataDir(resolvedDataDir, input));
    },
    summary: checkpointTreeSummary
  });
}
