...
- 专业格式转换 profile 统一由 `external-services/knowledge-distillation-service/format-conversion-profiles.json` 管理，协议为 `pact.external-knowledge-distillation.format-conversion-profiles.v1`，策略为 `singleton-format-conversion-profile-registry.v1`；服务启动时必须校验 profile 绑定到已有 route，且 parser stages 均来自 parser strategy registry。
- 真实模型蒸馏 profile 统一由 `external-services/knowledge-distillation-service/model-distillation-profiles.json` 管理，协议为 `pact.external-knowledge-distillation.model-distillation-profiles.v1`，策略为 `singleton-model-distillation-profile-registry.v1`；服务启动时必须校验 profile 要求真实模型调用、禁止内置 fallback，并声明 gateway request fields、system prompt、分类蒸馏 group map、输出约束和采样参数。
- 建立统一路由对象：
