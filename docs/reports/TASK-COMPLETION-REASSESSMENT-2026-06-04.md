...
| T12 provider mode、receipt 和 iCloud/OneDrive live | `contract-only` | `v001-cloud-drive-e2e` 通过，但当前证据仍不能证明 OneDrive remote-live OAuth 上传/下载，也不能证明 iCloud local-live smoke。 | 补 OneDrive real OAuth smoke；补 iCloud local-live receipt；缺凭据时继续标 `blocked/contract-only`。 |
