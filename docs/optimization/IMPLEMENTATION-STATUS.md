...
| Topic | Resolution |
| --- | --- |
| Local Prolly mutation rechunking | Implemented path-local mutation for `put` and `delete`; large-index tests assert mutation write counts remain far below full rebuild writes. |
...
- Prolly node roots, membership/non-membership Prolly-path proofs, canonical raw key ordering across reloads, cross-leaf non-membership forgery rejection, multi-chunk and non-aligned range diff traversal without full-tree fallback, boundary-local mutation equivalence to full rebuild, bounded scan/prefix traversal, `after` pagination, and limit clamp behavior;
- incremental workspace state-root mutation, touched-key proof verification, low single-key mutation write counts, and state-root retention through in-memory compaction;
