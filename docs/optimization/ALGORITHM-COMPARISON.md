...
| Dolt | Same data structure family: Prolly tree over ordered key/value entries. | Pactium uses protocol-defined key/value-derived boundaries, not Dolt's rolling splitter or storage engine. | Index roots point to content-addressed Prolly nodes, and mutation/diff use path-copying plus shared-node structure. |
...
| Index mutation | Index mutation rebuilt full snapshots. | Dolt chunker mutates ordered trees while reusing unchanged chunks. | Closed: `put`, `delete`, and `mutate` use path-copying, rewrite affected leaves and necessary ancestors, collapse single-child roots, and reuse unchanged subtrees. |
...
| Non-membership proof | Proofs relied on nearest-key membership material. | Prolly/B-tree style proofs include node ranges and boundary keys from the path. | Closed: compact non-membership proofs include the containing leaf, boundary keys, and compact path descriptor refs. |
| Multiproof/range proof | Multiple key proofs repeated sibling path descriptors. | Merkle multiproofs and range proofs share path material across related claims. | Closed: membership multiproofs, range proofs, and proof-material descriptor-table dedupe reduce repeated sibling descriptors. |
...
2. Verifiable indexes use content-addressed Prolly nodes, path-copying mutation, bounded scan/prefix, shared-node diff, multiproofs, range proofs, and descriptor-table proof compaction.
