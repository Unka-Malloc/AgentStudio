...
| Storage | `src/storage/storage-port.js`, `src/storage/local-json-storage-port.js`, `src/storage/sqlite-capability.js`, `src/storage/sqlite-storage-port.js` | Manifest-bound storage factory, JSON protocol object backend plus content-addressed block records, SQLite capability detection across npm/CLI/platform package managers, SQLite adapter through storage-capable providers (`node:sqlite` or optional npm `better-sqlite3`), DFS by `refs`, latest-schema-only directory boundary, write locking or SQLite transactions, cache refresh, and in-memory pruning helpers. |
