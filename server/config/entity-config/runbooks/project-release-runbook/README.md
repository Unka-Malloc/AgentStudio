...
When the user asks to release the current version (currently `v0.0.1`), the
agent MUST follow these exact steps:
...
4. **Generate Release Assets:**
   ...
   ```
   If you only need specific platforms, you can pass `-- --platforms=linux-x64,darwin-arm64` (x86_64 is still `linux-x64`).
...
5. **Upload Assets to GitHub Release:**
   ...
   Before running this command, copy `.github/RELEASE_TEMPLATE.md` to a temporary file such as `/tmp/release_notes.md`, fill in the highlights and details of the release, and then run:
   ```bash
