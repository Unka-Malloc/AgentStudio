...
assert.match(serviceSource, /SERVICE_HOST \|\| "127\.0\.0\.1"/, "service must bind to loopback by default");
assert.match(serviceSource, /const REQUIRE_API_TOKEN = envFlag\("PACT_EXTERNAL_KD_REQUIRE_API_TOKEN", !ALLOW_UNAUTHENTICATED_DEV\)/, "service must require API auth by default outside explicit unauthenticated dev");
assert.match(serviceSource, /PACT_EXTERNAL_KD_ALLOW_DATA_DIR_INPUTS/, "service must require an explicit gate before DATA_DIR becomes an input root");
assert.doesNotMatch(serviceSource, /const INPUT_ROOTS = Array\.from\(new Set\(\[\s*DATA_DIR,/, "service state directory must not be the default input root");
...
assert.match(dockerfile, /ENV PACT_EXTERNAL_KD_INPUT_ROOTS=\/inputs/, "container runtime must separate mounted input roots from service state");
assert.match(dockerfile, /chown -R pactkd:pactkd \/data \/inputs \/app/, "container writable paths must be owned by the service user");
...
async function assertDefaultRequiresToken() {
  const port = await freePort();
  const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "pact-external-kd-gate-default-token-"));
  const child = spawn(process.execPath, [serviceEntry], {
    cwd: repoRoot,
    env: {
      ...process.env,
      ...
      PACT_EXTERNAL_KD_API_TOKEN: "",
      SERVICE_API_TOKEN: "",
      PACT_EXTERNAL_KD_REQUIRE_API_TOKEN: "",
      PACT_EXTERNAL_KD_ALLOW_UNAUTHENTICATED_DEV: "0"
    },
    stdio: ["ignore", "pipe", "pipe"]
  });
  ...
  try {
    assert.notEqual(code, null, "service must not keep running by default without a token");
    assert.match(output, /PACT_EXTERNAL_KD_API_TOKEN must be set/, "default startup failure must identify the missing auth configuration");
  } finally {
    await stopService(child);
    await fs.rm(dataDir, { recursive: true, force: true });
  }
...
await assertDefaultRequiresToken();
...
try {
  ...
  assert.equal(Object.hasOwn(health.payload, "dataDir"), false, "public health must not disclose the service data directory");
  assert.equal(Object.hasOwn(health.payload, "runtimeDoctor"), false, "public health must not disclose runtime doctor details");
  ...
  assert.equal(Object.hasOwn(runtimeHealth.payload, "runtimes"), false, "public runtime health must not disclose dependency command paths");
  assert.equal(Object.hasOwn(runtimeHealth.payload, "summary"), false, "public runtime health must not disclose dependency inventory details");
