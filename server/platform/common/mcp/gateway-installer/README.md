# Pact MCP Gateway Installer

Release-packaged installer for connecting local AI agents to a Pact MCP HTTP endpoint.

This source belongs to the Pact server-side MCP Gateway implementation at
`server/platform/common/mcp/gateway-installer/`. The published package keeps the
`pact-mcp-connector` name as a lightweight installer artifact; it does not carry
the Pact server runtime, SQLite data, KnowledgeCore, or UI.
...
Cursor (`cursor`), and Hermes Agent (`hermes`).
...
Use `--token-stdin` only when an operator has pre-issued a custom grant for a
specific MCP client. The token is written into that MCP client configuration; it
is not a general-purpose Tool Management HTTP execute credential.
