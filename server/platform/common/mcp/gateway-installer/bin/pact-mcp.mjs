...
async function listMacApplicationPaths() {
...
async function notifyLocalMcpUninstall(options, { targets = [] } = {}) {
  ...
  const explicitToken = await resolveToken(options, { required: false });
  const token = explicitToken ||
    (await requestLocalMcpGrant(options, { targets: targetList, autoUpdate: false })).token;
