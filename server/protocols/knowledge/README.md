...
| Evaluation and gates | Replay maintained or generated cases against the knowledge agent and calculate retrieval quality gates. | `knowledge.evaluation.runs.create`, `knowledge.evaluation.runs.list`, `knowledge.evaluation.runs.get`, `knowledge.playbook_sets.evaluation.runs.create`; legacy `knowledge.skills.evaluation.runs.create` remains an alias. | AgentEvaluationRuntime and AgentLibraryPlaybookRuntime own run storage and metrics. | Not connected to Pact Work Queue. |
| Evolution governance | Run the closed loop that collects feedback, attributes failures, proposes candidates, evaluates regressions, publishes canaries, and supports rollback. | `knowledge.evolution.runs.create`, `knowledge.evolution.deployments.*`, `knowledge.hierarchy.audit`, `knowledge.playbook_sets.deployments.*`; legacy `knowledge.skills.deployments.*` remains an alias. | KnowledgeEvolutionRuntime orchestrates KnowledgeCore, evaluation, model decision, distillation, GoldenRule, and Playbook runtimes. | Not connected to Pact Work Queue; it is a compound workflow that may call learning, evaluation, and distillation. |
| Agent production | Use the knowledge base to create answer, exploration, summary, or Playbook artifacts while preserving evidence refs and review boundaries. | `knowledge.agent_explore.runs.create`, `knowledge.summarization.runs.create`, `knowledge.summarization.runs.approve`, `knowledge.retrieval_playbook.*`, `knowledge.playbooks.*`, `knowledge.playbook_framework.*`; legacy `knowledge.agent_skill.*` and `knowledge.skills.*` remain aliases. | AgentWorkspace, ContextRuntime, Agent Exploration, SummarizationRuntime, Retrieval Playbook runtime, and AgentLibraryPlaybookRuntime own runs, artifacts, private state, and proposals. | Not connected to Pact Work Queue; some calls can run local async/background modes. |
...
AgentLibrary built-in abilities use Playbook vocabulary. `knowledge.retrieval_playbook.*`
is the public Retrieval Playbook protocol (`v0.0.1:knowledge:retrieval-playbook-1`),
`knowledge.playbooks.*` is the public AgentLibrary Playbook protocol
(`v0.0.1:knowledge:playbook-1`), and `knowledge.playbook_framework.*` is the
Playbook Framework protocol (`v0.0.1:knowledge:playbook-framework-1`). Historical
`knowledge.agent_skill.*`, `knowledge.skills.*`, and `knowledge.skills.evaluation.*`
names are compatibility aliases only and must not be used as new Skill Hub
concepts.
...
| Distillation result export | `external.knowledge.distillation.artifacts.export`, `GET /api/external/knowledge/distillation/runs/:runId/artifacts/:artifactId`, `external knowledge distillation export --id RUN_ID` | Export refined self-contained distillation results as Markdown, DOCX, HTML, PDF, or another supported format via the external knowledge distillation service; the document must be understandable outside Pact. |
...
Knowledge distillation is maintained as the independently deployed `external.knowledge.distillation` service. The embedded workbench/runtime implementation has been completely removed from the server runtime path. Legacy `knowledge.distillation.*` and `knowledge.distillation.workbench.*` operations no longer exist; all distillation requests must use `external.knowledge.distillation.*` operations.
